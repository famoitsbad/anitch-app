...
const ECZEMA_DURATIONS = ['Less than 1 year', '1–3 years', '3–5 years', '5–10 years', '10+ years']
const ECZEMA_DURATIONS_ZH = ['少於1年', '1–3年', '3–5年', '5–10年', '10年以上']

export default function Auth() {
  const { t, lang, switchLang } = useApp()
  const [mode, setMode] = useState('login')
  const [step, setStep] = useState(1) // 1=credentials, 2=demographics
  ...
  async function handleStep1(e) {
    ...
    if (mode === 'login') {
      ...
      if (!agreed) { setError(isZh ? '請同意私隱政策以繼續' : 'Please agree to the Privacy Policy to continue'); return }
      setStep(2)
    }
  }
  ...
  async function handleStep2(e) {
    ...
    if (data.user) {
      await supabase.from('profiles').insert({
        ...
        age_range: ageRange, gender, eczema_duration: eczemaDuration
      })
    }
    setLoading(false)
  }
  ...
  return (
    <div style={s.wrap}>
      ...
      <div style={s.content}>
        <div style={s.card}>
          <div style={s.tagline}>{isZh ? '從濕疹中解放' : 'FREEDOM FROM ECZEMA'}</div>

          {/* STEP 1 */}
          {(mode === 'login' || step === 1) && (
            <>
              <div style={s.headline}>
                {mode === 'login' ? (isZh ? '歡迎回來' : 'Welcome back') : (isZh ? '建立帳號' : 'Create Account')}
              </div>
              <div style={s.subline}>
                {mode === 'login' ? (isZh ? '您的個人濕疹日記' : 'Your personal eczema diary') : (isZh ? '我們應該怎樣稱呼您？' : 'How would you like us to call you?')}
              </div>
              <form onSubmit={handleStep1} style={s.form}>
                ...
                {/* Privacy agreement for signup */}
                {mode === 'signup' && (
                  <div style={s.privacyBox}>
                    <div style={s.privacyRow} onClick={()=>setAgreed(!agreed)}>
                      <div style={{...s.checkbox, ...(agreed?s.checkboxOn:{})}}>
                        {agreed && <span style={{color:'white',fontSize:'12px'}}>✓</span>}
                      </div>
                      <div style={s.privacyText}>
                        {isZh
                          ? '我同意私隱政策。登入即代表您同意我們的私隱政策——所有數據均會私密保存，絕不會出售或分享給第三方。'
                          : 'By signing up, I agree to the Privacy Policy — all data is saved privately and will never be sold or shared with third parties.'
                        }
                      </div>
                    </div>
                  </div>
          ...
          {mode === 'signup' && step === 2 && (
            <>
              ...
              <div style={s.subline}>{isZh ? '這些資料有助於我們改善您的體驗（可選填）' : 'This helps us personalise your experience (optional)'}</div>
              <form onSubmit={handleStep2} style={s.form}>
                {/* Age */}
                <div style={s.field}>
                  <label style={s.label}>{isZh ? '年齡範圍' : 'Age Range'}</label>
                  <div style={s.optionGrid}>
                    {(isZh ? AGE_RANGES_ZH : AGE_RANGES).map((a, i) => (
                      <div key={a} style={{...s.optionBtn,...(ageRange===(isZh?AGE_RANGES_ZH:AGE_RANGES)[i]?s.optionActive:{})}}
                        onClick={()=>setAgeRange((isZh?AGE_RANGES_ZH:AGE_RANGES)[i])}>
                        {a}
                      </div>
                    ))}
                  </div>
                ...
                <div style={s.field}>
                  ...
                  <div style={s.optionRow}>
                    {(isZh ? GENDERS_ZH : GENDERS).map((g) => (
                      <div key={g} style={{...s.optionBtn,...(gender===g?s.optionActive:{})}} onClick={()=>setGender(g)}>{g}</div>
                    ))}
                  </div>
                </div>
                {/* Eczema duration */}
                <div style={s.field}>
                  ...
                  <div style={s.optionGrid}>
                    {(isZh ? ECZEMA_DURATIONS_ZH : ECZEMA_DURATIONS).map((d) => (
                      <div key={d} style={{...s.optionBtn,...(eczemaDuration===d?s.optionActive:{})}} onClick={()=>setEczemaDuration(d)}>{d}</div>
                    ))}
                  </div>
                </div>
                {error && <div style={s.error}>{error}</div>}
                ...
                <button type="button" style={s.skipBtn} onClick={handleStep2}>
                  {isZh ? '跳過' : 'Skip for now'}
                </button>
              </form>
            </>
          )}
...
const s = {
  ...
  langBtn:{padding:'4px 12px',borderRadius:'4px',border:'1px solid rgba(255,255,255,0.3)',background:'transparent',fontSize:'11px',cursor:'pointer',color:'rgba(255,255,255,0.7)',fontWeight:'600',letterSpacing:'0.05em'},
  ...
  privacyBox:{background:theme.greenLight,borderRadius:'8px',padding:'12px',border:`1px solid ${theme.greenSoft}`},
  privacyRow:{display:'flex',gap:'10px',alignItems:'flex-start',cursor:'pointer'},
  ...
  optionGrid:{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'6px'},
  optionRow:{display:'flex',gap:'6px',flexWrap:'wrap'},
  optionBtn:{padding:'8px 10px',borderRadius:'6px',border:`1.5px solid ${theme.border}`,fontSize:'12px',color:theme.textSecondary,cursor:'pointer',textAlign:'center',background:'white',fontWeight:'500',transition:'all 0.15s'},
  optionActive:{background:theme.green,borderColor:theme.green,color:'white',fontWeight:'700'},
